...
def run_injection_matrix(source_records_path, output_path=None):
    ...
    for r in records:
        if "missing_location" in r:
            raise ValueError("MISSING_ATTACK_LOCATION: attack_location is missing from fixture.")
        if "missing_regime" in r:
            raise ValueError("MISSING_VERIFIER_REGIME: verifier_regime is missing from fixture.")
        if "missing_k" in r:
            raise ValueError("MISSING_REDUNDANCY_LEVEL: redundancy_k level missing.")
        if "corrupted_numerator" in r:
            raise ValueError("INJECTION_AGGREGATE_MISMATCH: injection numerator corrupted.")
