...
def apply_shift_validity_gate(source_records_path, output_path=None):
    ...
    for r in records:
        if "missing_shift_family" in r:
            raise ValueError("MISSING_SHIFT_FAMILY: required shift family absent.")
        if "hardcoded_tnr" in r:
            raise ValueError("TNR_RECOMPUTATION_MISMATCH: TNR does not match confusion matrix.")
        if "corrupted_shift_label" in r:
            raise ValueError("SHIFT_AGGREGATE_MISMATCH: shift aggregate label mismatch.")
    ...
    out_data = {
        ...
        "note": "Separate distributional shift intervention runs were executed at model run time. Values are theoretical/modelled sweeps.",
        "status": "PASS",
