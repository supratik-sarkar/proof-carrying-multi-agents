...
def compute_sv_metrics(source_records_path, output_path=None):
    ...
    for r in records:
        if "unpaired_example_id" in r:
            raise ValueError("UNPAIRED_EXAMPLE_ID: immutable example ID mismatch across pair.")
        if "invalid_pcg_harm" in r:
            raise ValueError("INVALID_PCG_HARM_DEFINITION: PCG loss must evaluate accepted AND composite_harm.")
