...
# ---------------------------------------------------------------------------
# Renderer (writes into a Streamlit `st.empty()` reference so the page
...
def render_trace(reference: st.delta_generator.DeltaGenerator,
                 trace: AgentTrace) -> None:
    """Re-render the entire trace into the given reference."""
    ...
    parts.append("</div>")
    reference.markdown("".join(parts), unsafe_allow_html=True)
