...
def render_byok_sidebar() -> tuple[str, str | None, str]:
    ...
    with st.sidebar:
        ...
        chosen_label = st.selectbox(
            ...
            index=cur_index,
            help=(
                "HF Inference (free tier) is the default and has no direct API cost. "
                "Premium providers require a supplied API key."
            ),
        )
        ...
        if info.requires_key:
            ...
            api_key = st.text_input(
                ...
                value=saved,
                type="password",
                reference="paste here — kept in browser session only",
                help=(
                    f"Get one at {info.docs_url}. Never logged or persisted; "
                    "wiped when the browser tab is closed."
                ),
            )
            ...
            if _os.environ.get("HF_TOKEN"):
                ...
                st.warning(
                    ...
                    "above (HF Inference, provided token) to enable Live Run."
                )
        ...
        with st.expander("Privacy & cost", expanded=False):
            st.markdown(
                ...
                "the supplied provider account directly; a typical demo run costs $0.001–$0.05.\n\n"
                "**Privacy.** API keys live only in `st.session_state` "
                "(the browser tab). They are NOT sent to this server, "
                "NOT logged, and disappear when the tab is closed.\n\n"
