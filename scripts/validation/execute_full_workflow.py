...
VAL_DIR = EVIDENCE_DIR / "validation"
...
res_src = subprocess.run([PYTHON_BIN, str(REPO_ROOT / "scripts" / "validation" / "verify_source_record_integrity.py")], capture_output=True, text=True)
...
res_proto = subprocess.run([PYTHON_BIN, str(REPO_ROOT / "scripts" / "validation" / "verify_protocol_completion.py")], capture_output=True, text=True)
...
res_clean = subprocess.run([PYTHON_BIN, str(REPO_ROOT / "scripts" / "validation" / "verify_clean_room.py")], capture_output=True, text=True)
...
res_mut = subprocess.run([PYTHON_BIN, str(REPO_ROOT / "scripts" / "validation" / "run_mutation_tests.py")], capture_output=True, text=True)
...
res_ast = subprocess.run([PYTHON_BIN, str(REPO_ROOT / "scripts" / "validation" / "audit_artifact_python.py")], capture_output=True, text=True)
...
res_exec = subprocess.run([PYTHON_BIN, str(REPO_ROOT / "scripts" / "validation" / "generate_execution_matrix.py")], capture_output=True, text=True)
...
tbl_path = EVIDENCE_DIR / "table_reconciliation" / "table_reconciliation_summary.json"
...
sv_path = EVIDENCE_DIR / "sv_decomposition" / "sv_decomposition.json"
sv_decomposition_status = "PASS" if (sv_path.exists() and json.loads(sv_path.read_text()).get("status") == "PASS") else "FAIL"

bm_path = EVIDENCE_DIR / "backend_manifest" / "backend_manifest_summary.json"
...
print("=================================================================")
print("=== FINAL VALIDATION AUDIT & REPRODUCIBILITY REPORT ===")
print("=================================================================")
