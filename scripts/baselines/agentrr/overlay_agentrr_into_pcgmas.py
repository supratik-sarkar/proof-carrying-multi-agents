...
AGG_PATH = Path("results/baselines/agentrr/r1_r5/aggregate_by_dataset_model.json")
AUDIT_PATH = Path("results/tables/csv/agentrr_overlay_audit.json")
...
CURRENT_OVERLAY_METHOD = "agentrr"
CURRENT_OVERLAY_SIDECAR = Path("results/tables/csv/agentrr_overlay_rows.jsonl")
...
def main() -> int:
    ...
    if not AGG_PATH.exists():
        raise SystemExit(f"Missing AgentRR aggregate: {AGG_PATH}")
    ...
    for row in rows:
        ...
        false_accept = finite(
            mean_stat(agg.get("R1_false_accept_proxy_rate_among_known")),
            None,
        )
        ...
        audit_coverage = finite(
            first_nonnull(
                ...
                mean_stat(agg.get("audit_coverage_of_observed_bad_accepts")),
            ),
            0.0,
        )

        json_success = finite(mean_stat(agg.get("json_parse_success_rate")), None)
        json_repair = finite(mean_stat(agg.get("json_repair_rate")), None)
        invalid_rate = finite(mean_stat(agg.get("invalid_response_rate")), None)
        ece_proxy = finite(mean_stat(agg.get("ece_proxy")), None)

        r2_accept = finite(mean_stat(agg.get("R2_quorum_accept_n")), None)
        ...
        token_multiplier = max(
            1.0,
            (tokens_abs or 1.0) / max(n_base, 1.0) / max(nocert_tok, 1.0),
        )
        ...
        corruption_types = agg.get("corruption_types_used")
        ...
        rr.update(
            {
                "agentrr_overlay_applied": True,
                "agentrr_artifact_source": "results/baselines/agentrr/r1_r5",
                "agentrr_implementation_mode": "agentrr_style_record_replay_adapter_llm_backend",
                "agentrr_backend_mode": backend_mode,
                "agentrr_accept_rate": accept,
                "agentrr_block_rate": block,
                "agentrr_verify_rate": verify,
                "agentrr_false_accept_proxy_rate": false_accept,
                "agentrr_harm_under_corruption_mean": harm_mean,
                "agentrr_harm_under_corruption_max": harm_max,
                "agentrr_audit_coverage": audit_coverage,
                "agentrr_audit_coverage_on_bad_accepts": audit_coverage,
                "agentrr_json_parse_success_rate": json_success,
                "agentrr_json_repair_rate": json_repair,
                "agentrr_invalid_response_rate": invalid_rate,
                "agentrr_ece_proxy": ece_proxy,
                "agentrr_corruption_types_used": json.dumps(corruption_types),
                "clean_harm_agentrr": harm_mean,
                "adv_harm_agentrr": harm_max,
                "harm_clean_agentrr": harm_mean,
                "harm_adv_agentrr": harm_max,
                "bound_gap_agentrr": max(0.0, 1.0 - (audit_coverage or 0.0)),
                "agentrr_bound_gap": max(0.0, 1.0 - (audit_coverage or 0.0)),
                "agentrr_r2_quorum_accept_n": r2_accept,
                "agentrr_r3_total_decision_flips": r3_flips,
                "latency_agentrr": latency,
                "tokens_agentrr": token_multiplier,
                "token_agentrr": token_multiplier,
                "tokens_agentrr_abs_total": tokens_abs,
                "agentrr_prompt_tokens_abs_total": prompt_tokens_abs,
                "agentrr_completion_tokens_abs_total": completion_tokens_abs,
                "utility_agentrr": accept,
            }
        )
    ...
    audit = {
        ...
        "agentrr_keys": [list(k) for k in sorted(by_key)],
    ...
    if matched == 0:
        raise SystemExit("No AgentRR rows matched paper_metrics.jsonl.")
