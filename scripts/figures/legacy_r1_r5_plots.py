...
def load_cells() -> list[dict]:
    p = json.loads(Path("results/tables/csv/paper_metrics.jsonl").read_text())
        ...
        for method_idx, method in enumerate(METHODS):
            ...
            for eps_idx, eps_adv in enumerate(eps_grid):
                ...
                for k_idx, k in enumerate(ks):
                    method_scale = float(cell.get("r2", {}).get("method_scale", {}).get(method, scale[method]))
                    risk = (rho + eps_adv) ** (k - 1) * (stressed_eps ** k) * method_scale
